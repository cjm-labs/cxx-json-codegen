    ...
    for (auto field : object) {
        ...
        if (key == "groups") {
            ...
            for (auto decoded_groups_entry : decoded_groups_object) {
                ...
                std::vector<std::int32_t> decoded_groups_mapped_value{};
                ...
                for (auto decoded_groups_element : decoded_groups_array) {
                    std::int32_t decoded_groups_value{};
                    using target_type = decltype(decoded_groups_value);
                    std::int64_t decoded_groups = 0;
                    runtime_error = decoded_groups_element.get_int64().get(decoded_groups);
                    if (runtime_error) {
                    ...
                    if (decoded_groups < target_min || decoded_groups > target_max) {
                    ...
                    decoded_groups_value = static_cast<target_type>(decoded_groups);
...
template <>
inline std::optional<::MapVectorValues>
from_json<::MapVectorValues>(
    std::string_view input,
    DecodeError& error) {
