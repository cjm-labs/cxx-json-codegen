...
user_type.qualified_name = "User", usertype.fields = {name_field, age_field};
...
assert(project.types[0].fields[0].json.kind == FieldTypeKind::String);

assert(project.types[0].fields[1].name == "age");
assert(project.types[0].fields[1].json.name == "age");
assert(project.types[0].fields[1].json.kind == FieldTypeKind::SignedInteger);

assert(project.types[0].fields[0].source_location.file == "/include/user.hpp");
